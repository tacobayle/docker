...
resource "null_resource" "add_nic_to_gw_network_nsx_external" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      /usr/local/bin/govc vm.network.add -vm "${var.external_gw.name}" -net "${var.vcenter_underlay.networks.nsx.external.name}"
    EOT
  }
}
...
resource "null_resource" "add_nic_to_gw_network_nsx_overlay" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      export GOVC_CLUSTER=${var.vcenter_underlay.cluster}
      export GOVC_INSECURE=true
      /usr/local/bin/govc vm.network.add -vm "${var.external_gw.name}" -net "${var.vcenter_underlay.networks.nsx.overlay.name}"
    EOT
  }
}
...
resource "null_resource" "add_nic_to_gw_network_nsx_overlay_edge" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      export GOVC_CLUSTER=${var.vcenter_underlay.cluster}
      export GOVC_INSECURE=true
      /usr/local/bin/govc vm.network.add -vm "${var.external_gw.name}" -net "${var.vcenter_underlay.networks.nsx.overlay_edge.name}"
    EOT
  }
}
...
resource "null_resource" "adding_ip_to_nsx_overlay_and_nsx_overlay_edge" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo \"            dhcp4: false\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            addresses: [${var.vcenter_underlay.networks.nsx.overlay.external_gw_ip}/${var.vcenter_underlay.networks.nsx.overlay.prefix}]\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            match:\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      ...
      "echo \"            dhcp4: false\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            addresses: [${var.vcenter_underlay.networks.nsx.overlay_edge.external_gw_ip}/${var.vcenter_underlay.networks.nsx.overlay_edge.prefix}]\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            match:\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
