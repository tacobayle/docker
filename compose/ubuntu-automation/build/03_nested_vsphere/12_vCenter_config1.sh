#!/bin/bash
#
source /build/bash/vcenter_api.sh
#
...
#}
#
token=$(/bin/bash /build/bash/create_vcenter_api_session.sh "$vsphere_nested_username" "$vcenter_domain" "$vsphere_nested_password" "$api_host")
...
load_govc_env
govc vm.network.change -vm $(jq -r .vcenter.name $jsonFile) -net $(jq -r .vcenter_underlay.networks.vsphere.management.name $jsonFile) ethernet-0 &
govc_pid=$(echo $!)
